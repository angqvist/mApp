...
double MC_totalEnergy(LatticeList &ll,  std::vector<NeighbourList> &nl) 
{
  ...
  for(int i=0; i<ll.get_original_atoms_count(); i++)
    {
      //energies=nl[i].getLocalEnergy(ll);
      
      //ret += energies[1]+ energies[2]+energies[3]+energies[4];
      ret += nl[i].getLocalEnergyForTotal(ll);
      //nl[i].getLocalEnergy(ll);///((double)ll.get_original_atoms_count());
    }
  return ret;
}
...
void MC_occupations(LatticeList ll, double &c6, double &k24, double &i16,std::string type)
{
  ...
  int nbrOfCells=ll.get_original_atoms_count()/46;
